...
ui <- fluidPage(
    ...
    sliderInput("fontsize", "Select the Font Size", min = 10, max = 40, value = 25, step = 1),
...
server <- function(input, output, session) {
  ...
  ggplot(penguins, aes_string(x = input$x, y = input$y, color="species")) + geom_point() + theme_linedraw(base_size = input$fontsize)
  })
}
...
ui <- dashboardPage(
  ...
  fluidRow(
  box(title = "Plot Options", width = 3,
  selectInput("x", "Select X Variable", choices = c("HuntCat", "RA_Apes", "RA_Birds", "RA_Rodents"), selected = "bill_length_mm"),
  selectInput("y", "Select Y Variable", choices = c("Evenesss_BirdSpecies", "Diversity_BirdSpecies", "Rich_MammalSpecies", "Diversity_MammalSpecies"), selected = "bill_depth_mm"),
   sliderInput("pointsize", "Select the Point Size", min = 1, max = 5, value = 2, step = 0.5)
  ), # close the first box
  box(title = "Plot of Penguins Data", width = 7,
  plotOutput("plot", width = "600px", height = "500px")
