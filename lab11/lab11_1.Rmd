...
deserts %>% 
  ...
  summarise(n=n())
```
#r interprets dates as numeric, it sees it as 1 thousand 9 hundred 77 which affects the plots.
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="darkviolet", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "blue", alpha = 0.4, color = "black")+
