...
```{r}
beachbugs_high <- beachbugs_long %>%
  group_by(site) %>%
  mutate(buglevel_high = max(buglevels, na.rm = T)) %>%
  arrange(site)
beachbugs_high
```

```{r}
beachbugs_normal <- beachbugs_high %>%
  group_by(site) %>%
  mutate(buglev_norm = buglevels/beachbugs_high) %>%
  arrange(site,year) %>%
  select(site, year, buglev_norm)
